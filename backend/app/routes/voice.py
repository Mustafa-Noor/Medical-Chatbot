...
@router.post("/reply")
async def reply_endpoint(payload: dict, db: AsyncSession = Depends(get_db), current_user=Depends(deps.get_current_user)):
...
@router.post("/process")
async def full_pipeline(audio: UploadFile, topic: str, db: AsyncSession = Depends(get_db), current_user=Depends(deps.get_current_user), session_id: str | None = None):
